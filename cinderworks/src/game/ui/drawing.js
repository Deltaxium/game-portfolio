...
export function drawValveMarker(scene, x, y, solved, options = {}) {
  const ringColor = solved ? palette.green : palette.amber;
  const centerColor = solved ? palette.brass : options.color || palette.red;
  const rotation = solved ? 45 : options.rotation || 0;
  ...
  const spokeGroup = scene.add.container(x, y).setAngle(rotation);
  spokeGroup.add(scene.add.line(0, 0, -13, 0, 13, 0, palette.cream).setLineWidth(3));
  spokeGroup.add(scene.add.line(0, 0, 0, -13, 0, 13, palette.cream).setLineWidth(3));
